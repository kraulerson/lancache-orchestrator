...
class TestEnumerateViaPrefill:
    """re-arch ③b: the prefill enumeration now looks up each prefilled app via
    the public Steam store appdetails API to get its type + name, upserts ONLY
    ...
    def _patch_settings(self, monkeypatch, *, budget=150, delay=0.0):
        ...
        monkeypatch.setattr(
            ...
            lambda: settings_mod.Settings(
                ...
                steam_enumerate_via_prefill=True,
    ...
    async def test_uses_cache_no_refetch(self, pool, monkeypatch):
        self._patch_settings(monkeypatch)
        await pool.execute_write(
            "INSERT INTO steam_app_info (app_id, app_type, name) "
            "VALUES ('440','game','Team Fortress 2')"
        )
    ...
    async def test_budget_limits_fetches(self, pool, monkeypatch):
        ...
        async def counting_fetch(app_id: int):
            calls.append(app_id)
            return {"type": "game", "name": f"Game {app_id}"}
