...
async def test_steam_purge_sets_validation_failed(pool):
    ...
    and lands as a downward transition, which is what makes a mass purge alarm."""
    ...
    t = await pool.read_one(
        "SELECT prior, new_status, downward FROM measurement_transitions WHERE game_id=?",
        (game_id,),
    )
    assert t == {"prior": "up_to_date", "new_status": "validation_failed", "downward": 1}
