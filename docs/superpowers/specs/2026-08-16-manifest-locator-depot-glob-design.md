...
**Date:** 2026-08-16
**Status:** Revised after adversarial review round 4 (2026-08-16) — see §9–§12. Core fix (§3) logically unchanged since round 1, but its code sample was itself wrong until round 4 — see §12.
...
**Round-3 verdict: REQUEST CHANGES** — a genuinely new gap (a deploy landing mid-sweep), an unspecified snapshot mechanism (replaced by reusing the sweep's own existing `evicted`/`recovered` metrics rather than inventing a new one), an under-justified threshold, a trivial citation slip, and a real process issue (this PR's branch was accidentally built on top of a separate, unmerged PR). Addressed; see §11.

**Round-4 verdict: REQUEST CHANGES** — round 3's branch fix had an unnoticed side effect: every citation into `steam.py`/`settings.py` throughout the document had been verified against the closed, wrong branch and never re-checked after the move, so six of them pointed at line numbers that don't exist on `main`. More seriously, §3's own fix code sample — the single most load-bearing snippet in the document — would have broadened `.shas` along with `.bin` if applied as literally shown, contradicting its own stated scope. Addressed; see §12. Four independent reviewers have now each verified every claim against the live source rather than trusting the document — all four found real issues, all four sets are now resolved below.
...
**Round-4 correction: the fix cannot be a single-line substitution as an earlier revision of this document showed.** The real loop, `manifest_locator.py:68-69` (current `main`), shares ONE glob call across both extensions, parameterized only by `ext`:

```python
for ext in _MANIFEST_EXTS:                              # ("bin", "shas")
    for path in v1.glob(f"{app_id}_{app_id}_*.{ext}"):
```

A naive replacement of that one line with `f"{app_id}_*.{ext}"` broadens **both** `.bin` and `.shas` — directly contradicting §3's own stated intent that only `.bin` widens. The loop has to branch per extension instead:
...
**Integration (`tests/agent/test_steam_purge.py`).** `_steam_chunk_paths` (`agent/routers/steam.py:244`) is shared verbatim by `/v1/steam/validate` and `/v1/steam/purge`; §7 below explains why purge's behavior changes too.
...
`_steam_chunk_paths` (`agent/routers/steam.py:244-319`) is the single shared source of manifest→cache-path enumeration for **both** `/v1/steam/validate` and `/v1/steam/purge` (stated in its own docstring, line 248-249) — it calls `locate_manifest_bins` directly, so §3's fix applies to both callers identically, with no code change needed in either router.

`steam_purge` (`agent/routers/steam.py:417-443`) applies no depot-level scoping of its own: "every enumerated chunk across all depots (no depot-scoping — purge_chunks no-ops on paths that aren't present)" (line 431-432 comment, verified verbatim in source). After this fix, purging any Steam game with a secondary depot under a differing group ID will delete that depot's chunk files too — files the old narrow glob never even enumerated, so purge silently left them behind before. This is the same correctness direction as the validate fix (purge should act on everything a game actually owns, not on whatever a buggy glob happened to see) and is not a new category of behavior, but it was entirely unexamined and untested in the original draft.

**Checked and confirmed safe against the one sharper risk this could imply — cross-game deletion via a shared depot:** the Steamworks Common Redistributables exclusion (`settings.steam_shared_redist_depots`, PR #245) is applied *inside* `_steam_chunk_paths` itself (`agent/routers/steam.py:295-302`), keyed by `depot_id`, before any path is added to what either validate or purge sees. It is unconditional on which glob found the file. Widening the `.bin` glob does not bypass or weaken this exclusion in any way — a depot that's supposed to be protected from purge stays protected. This was verified by reading the exclusion's exact position in the shared function, not assumed from the original PR's intent.
...
| 4 | §7's citation for the "no depot-scoping" purge comment was off by one line (`516-518` vs. actual `516-517`) | Fixed at the time — accurate for the branch round 3 actually reviewed. **Invalidated again by finding 5 in this same round; see round-4 finding 1** |
| 5 | **Process issue, not a content defect:** the PR this document lives in was not actually "one file, no code" as claimed — the branch was accidentally created from the tip of the still-open, unmerged #275 rather than from `main`, so the PR's diff against `main` included four unrelated code commits from that separate branch | Verified independently (`git merge-base` against `main`) before acting: confirmed. #276 closed with an explanation; content moved via cherry-pick (identical tree, verified) to a new branch created correctly from `main`'s current tip, opened as #277. #275 unaffected. **This fix had an unnoticed side effect — see round-4 finding 1** |

Round-3 verdict: REQUEST CHANGES.

## 12. Adversarial review round 4 (2026-08-16)

A fourth, independent reviewer first confirmed the branch-cleanliness fix from round 3 (`git merge-base` against `origin/main`, and a stat-diff showing exactly one file changed — both re-verified independently here too, both hold). It then re-verified every citation in the document from scratch — the fourth independent pass over the same claims — and found the branch move's actual consequence, which nobody (including two prior review rounds and the author) had caught:

| # | Finding | Resolution |
|---|---|---|
| 1 | **Root cause of round-3 finding 4, much larger than "off by one":** every citation into `agent/routers/steam.py` and `settings.py` throughout the document — six citations across §4, §7 — was verified against the **closed #276 branch** (built on the unmerged #275's extra commits), not against `main`. `_steam_chunk_paths` is at `main` line 244, not 329; its docstring at 248-249, not 333-335; the shared-redist exclusion at 295-302, not 380-387; `steam_purge` at 417-443, not 501-528 (`steam.py` is 443 lines total on `main` — line 501 doesn't exist); `sweep_batch_size` at `settings.py:250`, not 256. Mechanism: round 3's branch-fix (finding 5, same round) moved the spec's *text* to the corrected branch via cherry-pick, but the *citations embedded in that text* were never re-checked against the real files the corrected branch actually has — files `sweep.py` and `jobs/reaper.py` were untouched by #275's extra commits, so citations into those happened to still be correct by coincidence; only `steam.py` and `settings.py`, which #275 *did* modify, drifted | All six citations corrected to their real `main` line numbers throughout §3, §4, §7 |
| 2 | **The single most load-bearing code sample in the document was wrong.** §3 proposed `for path in v1.glob(f"{app_id}_*.{ext}"):` as a drop-in replacement for the real loop at `manifest_locator.py:68-69`, which shares ONE glob call across both extensions (`for ext in _MANIFEST_EXTS: for path in v1.glob(f"{app_id}_{app_id}_*.{ext}"):`). Applied literally, the proposed one-liner broadens `.shas` too — directly contradicting §3's own claim that only `.bin` widens | §3 rewritten: the loop must branch per extension (`pattern = f"{app_id}_*.{ext}" if ext == "bin" else f"{app_id}_{app_id}_*.{ext}"`), shown explicitly against the real current loop structure, not a paraphrase of it |
| 3 | Minor precision gap in §6's decision rule: an anomalously high `evicted` count is attributed only to "a bug in the widened glob," never to a genuine concurrent NAS-side eviction event — which this project already has a dedicated monitor for (cache-catcher) | Not independently re-verified as blocking; noted for awareness. Does not change the rule's action (halt and inspect either way) |

Everything else — the deploy-guard predicate (`state='running'`, confirmed correct: a merely-`queued` sweep poses no race since it's simply claimed by the new code after restart), the `evicted`/`recovered` semantics, the "no denominator shift from concurrent `library_sync`" reasoning, and the "20 games" threshold's honesty as a stated judgment call — was independently re-verified and held up without changes needed.

Round-4 verdict: REQUEST CHANGES. This revision addresses every item above.

## 13. Open questions

None block sending this back for a fifth review round. One item is explicitly the Orchestrator's call rather than a default I've picked:
