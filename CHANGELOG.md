...
Reviewing the security fix above found defects in it. Recorded here because they are CI/configuration changes, and because a fix that reintroduces the flaw it closes is worth naming.
...
- **Root cause: nothing ever compared the two lockfiles.** `pip-audit` scanned `requirements.txt` only (`ci.yml:129`), so no check could see a CVE in a dev pin, and no check compared the files to each other. Both gaps are closed here — the audit now covers both lockfiles in one invocation, and `tests/test_lockfile_consistency.py` asserts the two agree. Verified the audit bites before adding it: against the pre-fix lockfile it reports exactly `idna 3.11 PYSEC-2026-215`; against the fixed one, clean.
...
Every finding above surfaced from adversarial review of the routine Dependabot PRs #285/#286/#287, and each was reproduced independently before being acted on. A further review of *this* change found more — see the Infrastructure entry below.
