...
- **Root cause: `pip-audit` scanned `requirements.txt` only** (`ci.yml:129`), so no check could ever see a CVE in a dev pin. A second `pip-audit --requirement requirements-dev.txt` step now runs alongside it. Verified the gate bites before adding it: run against the pre-fix lockfile it reports exactly `idna 3.11 PYSEC-2026-215`; against the fixed one, clean.
- **Compounding cause: `pip-compile` had been unrunnable since 2026-08-09.** pip-tools 7.6.0 cannot import `stdlib_pkgs` from the `pip==26.2.1` this repo pins — reproduced in the project `.venv` as `ImportError: cannot import name 'stdlib_pkgs' from 'pip._internal.utils.compat'`. So even a maintainer who wanted to regenerate the dev lockfile could not, and no CI job invokes `pip-compile`, so nothing went red. PR #287 (pip-tools 7.6.1) repaired it; this change is the first regeneration since.
- **The licence gate had the same blind spot** — it installed `requirements.txt` only, so a dev dependency adopting GPL/AGPL would pass green. It now installs `requirements-dev.txt` too. That exposed a real divergence: `tests/test_licenses.py` allows the SPDX short form `MPL-2.0` (which `hypothesis` reports) but the CI allowlist carried only the long form `Mozilla Public License 2.0 (MPL 2.0)`. The two allowlists are duplicated by hand and had silently drifted apart; `MPL-2.0` is added to the CI copy. Gate verified passing against the full dev+runtime environment before the change was written.
...
All three findings surfaced from adversarial review of the routine Dependabot PRs #285/#286/#287 and were each reproduced independently before being acted on.
